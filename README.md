...
アプリの構造はこんな感じ。担当するエリアを完全に分けているので、お互いのコードがコンフリクトすることはない

* **メイン画面（API連携・分析）：** 佐藤担当(裏の繋ぎ込みは全部やります)
* **クイズ投稿機能（CRUD）：** 佐藤担当(問題文やヒント画像の保存など)
* **カテゴリ管理機能（CRUD）：** 飯塚担当
...
まずはプロジェクトを自分のPCに持ってきて、動かせる状態にする。ターミナルで1行ずつ実行。

### 1. データのダウンロードと移動
```bash
git clone https://github.com/ya-sato-mp/quiz_app.git
cd quiz_app
...
**※重要：`main` ブランチで直接コードを書くのはNG！**

### ① 自分の作業ブランチを作る（最初の一回だけ）

作業を始める前に、必ずこのコマンドを打って自分専用の部屋に切り替え。
...
GitHubのブラウザ画面を開くと「**Compare & pull request**」という緑色のボタンが出る。それを押してプルリクを送れば作業完了

---

##  飯塚担当：『カテゴリ管理（CRUD）』

クイズのジャンル(例：「アニメ」「歴史」「IT」など)を登録・管理する機能。
...
ルート(道路)はもう作ったので、以下のファイルだけを編集・作成してください。
